            ...
            for attempt in range(attempts):
                try:
                    ...
                    if attempt < attempts - 1:
                        ...
                        logger.warning(
                            f"Attempt {attempt + 1}/{attempts} failed for {func.__name__}. Retrying in {sleep_time:.2f} seconds..."
                        )
...
class PolygonAPI:
    ...
    async def get_stock_snapshot(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if snapshot and hasattr(snapshot, "ticker") and snapshot.ticker:
                ...
                raise PolygonInvalidResponseError(
                    f"Invalid snapshot response for {symbol}: {snapshot}"
                )
    ...
    async def get_intraday_bars(
        self, symbol: str, minutes: int = 1, days: int = 1
        ...
        try:
            ...
            raise PolygonDataError(
                f"Unexpected error fetching intraday bars for {symbol}: {e}"
            ) from e
    ...
    async def get_ticker_details(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if ticker_details and hasattr(ticker_details, "ticker"):
                ...
                ticker_details
                and hasattr(ticker_details, "status")
                and ticker_details.status == "NOT_FOUND"
            ):
                raise PolygonSymbolNotFoundError(f"Symbol not found: {symbol}")
            else:
                raise PolygonInvalidResponseError(
                    f"Invalid ticker details response for {symbol}: {ticker_details}"
                )
            ...
            raise PolygonDataError(
                f"Unexpected error fetching ticker details for {symbol}: {e}"
            ) from e

    @retry_async(attempts=3, delay=10)
    async def get_stock_universe(
        self, type: str = "cs", active: bool = True
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def subscribe_to_symbols(self, symbols: List[str]):
        ...
        try:
            ...
            if self.ws_client:
                ...
                logger.info(
                    f"Subscribed to {len(symbols)} symbols: {', '.join(symbols[:5])}{' ...' if len(symbols) > 5 else ''}"
                )
                return True
        except Exception as e:
    ...
    async def get_gainers_losers(self, limit: int = 20) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            gainers = list(
                self.rest_client.get_snapshot_gainers_losers(direction="gainers", limit=limit)
            )
            losers = list(
                self.rest_client.get_snapshot_gainers_losers(direction="losers", limit=limit)
            )
                    ...
                    gainers_formatted.append(
                        {
                            ...
                            "price": ticker.ticker.day.close
                            if hasattr(ticker.ticker.day, "close")
                            else ticker.ticker.min.close,
                            "change": ticker.ticker.todaysChange,
                            "change_percent": ticker.ticker.todaysChangePerc,
                            "volume": ticker.ticker.day.volume
                            if hasattr(ticker.ticker.day, "volume")
                            else ticker.ticker.min.volume,
                        }
                    )
            ...
            result = {"gainers": gainers_formatted, "losers": losers_formatted}
                ...
                try:
                    ...
                    if aggs:
                        ...
                        if premarket_aggs:
                            ...
                            high_price = (
                                max([agg.high for agg in premarket_aggs])
                                if premarket_aggs
                                else None
                            )
                            low_price = (
                                min([agg.low for agg in premarket_aggs]) if premarket_aggs else None
                            )
                            volume = (
                                sum([agg.volume for agg in premarket_aggs]) if premarket_aggs else 0
                            )
    ...
    async def get_trading_status(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise PolygonDataError(
                f"Unexpected error fetching trading status for {symbol}: {e}"
            ) from e
    ...
    async def get_previous_close(self, symbol: str) -> Optional[float]:
        ...
        try:
            ...
            raise PolygonDataError(
                f"Unexpected error fetching previous close for {symbol}: {e}"
            ) from e
    ...
    async def get_dividends(self, symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Fetch dividend data
            dividends = list(
                self.rest_client.get_dividends(
                    ticker=symbol, ex_dividend_date_gte=start_str, ex_dividend_date_lte=end_str
                )
            )
    ...
    async def get_splits(self, symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Fetch split data
            splits = list(
                self.rest_client.get_splits(
                    ticker=symbol, execution_date_gte=start_str, execution_date_lte=end_str
                )
            )
    ...
    async def get_news(self, symbol: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Fetch news
            news = list(
                self.rest_client.get_ticker_news(
                    ticker=symbol,
                ...
                results.append(
                    {
                        ...
                        if hasattr(article.publisher, "name")
                        else None,
                        "title": article.title,
    ...
    async def get_options_chain(self, symbol: str, expiration_date: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            options = list(
                self.rest_client.get_options_chain(symbol, expiration_date=expiration_date)
            )
            ...
            for option in options:
                option_data = {
                    ...
                    "last_price": option.last_trade.price
                    if hasattr(option, "last_trade") and option.last_trade
                    else None,
                    "bid": option.bid if hasattr(option, "bid") else None,
                    "ask": option.ask if hasattr(option, "ask") else None,
                    "volume": option.day.volume
                    if hasattr(option, "day") and hasattr(option.day, "volume")
                    else 0,
                    "open_interest": option.open_interest
                    if hasattr(option, "open_interest")
                    else 0,
                    "implied_volatility": option.implied_volatility
                    if hasattr(option, "implied_volatility")
                    else None,
                    "delta": option.greeks.delta
                    if hasattr(option, "greeks") and hasattr(option.greeks, "delta")
                    else None,
                    "gamma": option.greeks.gamma
                    if hasattr(option, "greeks") and hasattr(option.greeks, "gamma")
                    else None,
                    "theta": option.greeks.theta
                    if hasattr(option, "greeks") and hasattr(option.greeks, "theta")
                    else None,
            ...
            result = {
                ...
                "expirations": [exp.expiration_date for exp in expirations]
                if "expirations" in locals()
                else [],
                "calls": calls,
                "puts": puts,
                "updated_at": pd.Timestamp.now().isoformat(),
            ...
            # Cache in Redis
            redis_client.set(
                f"stocks:options:{symbol}:{expiration_date}", result, expiry=1800
            )  # 30-minute expiry
