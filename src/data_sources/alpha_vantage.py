...
class AlphaVantageAPI:
        ...
        **kwargs,
        ...
        try:
            ...
            if "Technical Analysis" not in data:
                logger.warning(
                    f"No technical indicator data found for {symbol} with indicator {indicator}"
                )
                return None
            ...
            if technical_key not in data:
                logger.warning(
                    f"Technical indicator data format unexpected for {symbol} with indicator {indicator}"
                )
                return None
            ...
            if not technical_data or len(technical_data) == 0:
                logger.warning(
                    f"Empty technical indicator data for {symbol} with indicator {indicator}"
                )
                return None
            ...
            for col in df.columns:
                try:
                    ...
                    logger.warning(
                        f"Error converting column {col} to numeric for {symbol} indicator {indicator}: {e}"
                    )
    ...
    async def get_company_overview(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if data.get("Symbol") != symbol:
                logger.warning(
                    f"Symbol mismatch in company overview data: requested {symbol}, got {data.get('Symbol')}"
                )

            # Data validation - check for essential fields
            essential_fields = ["Name", "Industry", "Sector", "MarketCapitalization"]
            missing_fields = [
                field for field in essential_fields if field not in data or not data[field]
            ]

            if missing_fields:
                logger.warning(
                    f"Missing essential fields in company overview for {symbol}: {missing_fields}"
                )
    ...
    async def get_income_statement(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            missing_fields = [
                ...
                if field not in latest_report or not latest_report[field]
            ]

            if missing_fields:
                logger.warning(
                    f"Missing essential fields in income statement for {symbol}: {missing_fields}"
                )
    ...
    async def get_balance_sheet(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if missing_fields:
                logger.warning(
                    f"Missing essential fields in balance sheet for {symbol}: {missing_fields}"
                )
    ...
    async def get_cash_flow(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if missing_fields:
                logger.warning(
                    f"Missing essential fields in cash flow for {symbol}: {missing_fields}"
                )
    ...
    async def get_news_sentiment(
        ...
        try:
            ...
            if "feed" not in data:
                logger.warning("No news sentiment data found for the specified parameters")
                return None

            # Validate that we have actual news items
            if not data["feed"] or len(data["feed"]) == 0:
                logger.warning("Empty news feed returned")
                return None
    ...
    async def get_symbol_news(self, symbol: str, limit: int = 10) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
                    ...
                    for ticker_sentiment in ticker_sentiments:
                        if ticker_sentiment.get("ticker") == symbol:
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"Non-numeric sentiment scores for {symbol} in news item"
                                )
                                relevance_score = 0
                                sentiment_score = 0
                            break
            ...
            formatted_news.sort(
                key=lambda x: (x["relevance_score"], x.get("time_published", "")), reverse=True
            )
    ...
    async def get_economic_indicator(self, indicator: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if "date" not in df.columns or "value" not in df.columns:
                logger.warning(
                    f"Missing required columns in economic indicator data for {indicator}"
                )
                return None
    ...
    async def get_treasury_yield(
        ...
        try:
            ...
            if "data" not in data:
                logger.warning(
                    f"No treasury yield data found for {maturity} with interval {interval}"
                )
                return None
                ...
                for sector, performance in sector_data.items():
                    try:
                        ...
                        if not isinstance(performance, str):
                            logger.warning(
                                f"Non-string performance value for {sector}: {performance}"
                            )
                            continue
...
async def fetch_alpha_vantage_data(
    symbol: str, interval: str = "daily", **kwargs
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if interval in ["1min", "5min", "15min", "30min", "60min"]:
            df = await alpha_vantage_client.get_intraday_prices(
                symbol, interval, kwargs.get("outputsize", "compact")
            )
        else:
            df = await alpha_vantage_client.get_daily_prices(
                symbol, kwargs.get("outputsize", "compact")
            )
