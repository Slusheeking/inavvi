...
def parse_market_analysis(response_content: str) -> Dict[str, Any]:
    ...
    if analysis["market_regime"] not in valid_regimes:
        logger.warning(
            f"Invalid market_regime value: {analysis['market_regime']}, defaulting to 'uncertain'"
        )
...
def parse_llm_response(response_content: str, task_type: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"error": str(e), "task_type": task_type, "raw_response": response_content}
...
def extract_market_analysis(response_content: str) -> Dict[str, Any]:
    ...
    if not structured_data:
        ...
        insights_pattern = (
            r"(?:key insights|key points|main takeaways)[:\n]+((?:[-•*]\s*[^\n]+\n?)+)"
        )
    ...
    if "overall_sentiment" in structured_data:
        analysis["overall_sentiment"] = (
            structured_data["overall_sentiment"].lower()
            if isinstance(structured_data["overall_sentiment"], str)
            else "neutral"
        )
    else:
...
def extract_trading_signals(response_content: str) -> List[str]:
    ...
    if structured_data:
        ...
        if "trading_signals" in structured_data and isinstance(
            structured_data["trading_signals"], list
        ):
...
def extract_structured_data(response_content: str) -> Dict[str, Any]:
    ...
    if not data:
        ...
        data = {"raw_text": response_content, "extracted_data": False}
