...
class PositionMonitor:
    ...
    async def validate_positions(self):
        ...
        for symbol, _position_data in positions.items():
            try:
                # Get current price
    ...
    async def monitor_positions(self):
        ...
        for symbol, position in positions.items():
            ...
            try:
                ...
                asyncio.create_task(
                    self.monitor_position(symbol, position, market_state, time_until_close)
                )
            except Exception as e:
    ...
    async def monitor_position(
        self, symbol: str, position: Dict[str, Any], market_state: str, time_until_close: float
    ):
        """
        ...
        try:
            ...
            if intraday_data is None or (
                isinstance(intraday_data, pd.DataFrame) and intraday_data.empty
            ):
            ...
            forced_exit = await self.check_forced_exit_conditions(
                symbol, position, market_state, time_until_close
            )
            ...
            ml_exit_triggered = (
                exit_recommendation.get("exit", False)
                or exit_recommendation.get("stop_loss_triggered", False)
                or exit_recommendation.get("take_profit_triggered", False)
                or exit_recommendation.get("trailing_stop_triggered", False)
                or exit_recommendation.get("time_stop_triggered", False)
            )

            if ml_exit_triggered:
                logger.info(
                    f"ML exit signal triggered for {symbol}: {exit_recommendation.get('reason', 'unknown')}"
                )
                ...
                if exit_decision["decision"] == "exit":
                    logger.info(
                        f"LLM confirmed exit for {symbol} with size {exit_decision['exit_size']}"
                    )
    ...
    async def get_exit_decision(
        ...
        if decision["decision"] == "exit":
            logger.info(
                f"Decision: EXIT {position_data['symbol']} with {decision['exit_size'] * 100:.0f}% of position"
            )
    ...
    async def calculate_optimal_exits(self):
        ...
        for symbol, position in positions.items():
            try:
                ...
                logger.debug(
                    f"Calculated optimal exits for {symbol}: stop=${optimal_stop:.2f}, target=${optimal_target:.2f}"
                )
            except Exception as e:
    ...
    async def get_portfolio_state(self) -> Dict[str, Any]:
        ...
        used_capital = sum(
            p.get("entry_price", 0) * p.get("quantity", 0) for p in positions.values()
        )
