...
class TradeExecutor:
    ...
    def __init__(self):
        ...
        self.alpaca = tradeapi.REST(
            self.alpaca_api_key, self.alpaca_api_secret, self.alpaca_base_url, api_version="v2"
        )
    ...
    async def check_pending_orders(self):
        ...
        try:
            ...
            for order in orders:
                logger.info(
                    f"Found open order: {order.id} for {order.symbol} ({order.side} {order.qty} @ {order.type})"
                )

                # Store order in Redis
                redis_client.set(
                    f"alpaca:orders:{order.id}",
                    {
                        "id": order.id,
                        "symbol": order.symbol,
                        "side": order.side,
        ...
        try:
            ...
            for symbol in missing_symbols:
                ...
                if position:
                    ...
                    logger.info(
                        f"Added position from Alpaca: {symbol} ({position_data['quantity']} @ ${position_data['entry_price']:.2f})"
                    )

            # Remove extra positions from our system
            for symbol in extra_symbols:
                logger.warning(
                    f"Found position in our system that doesn't exist in Alpaca: {symbol}"
                )
            ...
            logger.info(
                f"Position sync complete: {len(alpaca_symbols)} positions from Alpaca, {len(our_symbols)} in our system"
            )
    ...
    async def make_trade_decision(self, candidate: dict):
        ...
        if decision["decision"] == "trade":
            logger.info(
                f"Decision: TRADE {candidate['symbol']} with {decision['position_size'] * 100:.0f}% position size"
            )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"Found existing position in {symbol}: {existing_position.qty} shares"
                    )
                except:
                    pass
                ...
                if self.trading_mode == "paper":
                    ...
                    logger.info(
                        f"Order submitted: {order.id} for {order.qty} shares of {symbol} at market"
                    )
                    ...
                    logger.info(
                        f"SIMULATION: Would submit market order for {quantity} shares of {symbol}"
                    )
                ...
                logger.info(
                    f"Trade executed for {symbol}: {quantity} shares at ~${current_price:.2f}"
                )
        ...
        for symbol, signal in signals.items():
            ...
            try:
                ...
                await self.execute_exit(
                    symbol, position, exit_size, signal.get("reason", "exit_signal")
                )
    ...
    async def execute_exit(
        self, symbol: str, position: Dict[str, Any], exit_size: float, reason: str
    ):
        """
        ...
        try:
            ...
            if self.trading_mode == "paper":
                ...
                logger.info(
                    f"Exit order submitted: {order.id} for {order.qty} shares of {symbol} at market"
                )
                ...
                # In live mode, log but don't actually submit
                logger.info(
                    f"SIMULATION: Would submit market sell order for {exit_quantity} shares of {symbol}"
                )
                ...
                if current_price > 0 and entry_price > 0:
                    ...
                    if total_trades > 0:
                        self.trading_stats["win_rate"] = (
                            self.trading_stats["successful_trades"] / total_trades
                        )
        ...
        try:
            ...
            for order in orders:
                if order.status in ["canceled", "rejected", "expired"]:
                    # Log the failure
                    logger.warning(
                        f"Order {order.id} for {order.symbol} {order.side} {order.qty} was {order.status}"
                    )
                    ...
                    if order.side == "buy":
                        ...
                        if (
                            ...
                            and position.get("order_id") == order.id
                        ):
                            # Delete the pending position
                            redis_client.delete_active_position(order.symbol)
                            logger.info(
                                f"Deleted pending position for {order.symbol} due to {order.status} order"
                            )
    ...
    async def process_filled_entry(self, order):
        ...
        if position and position.get("order_id") == order.id:
            ...
            logger.info(
                f"Updated position for {symbol}: {position['quantity']} shares @ ${position['entry_price']:.2f}"
            )
            ...
            logger.warning(
                f"Filled order {order.id} for {symbol} doesn't match any pending position"
            )
            ...
            logger.info(
                f"Created new position from fill for {symbol}: {position_data['quantity']} shares @ ${position_data['entry_price']:.2f}"
            )
    ...
    async def process_filled_exit(self, order):
        ...
        if position:
            ...
            if exit_quantity >= current_quantity:
                # Full exit
                logger.info(
                    f"Full exit filled for {symbol}: {exit_quantity} shares @ ${float(order.filled_avg_price):.2f}"
                )
                ...
                logger.info(
                    f"Partial exit filled for {symbol}: {exit_quantity} shares @ ${float(order.filled_avg_price):.2f}"
                )
                logger.info(f"Remaining position: {remaining_quantity} shares")
            ...
            logger.warning(
                f"Filled exit order {order.id} for {symbol} but no matching position found"
            )
    ...
    async def get_trading_stats(self):
        ...
        profitable_trades = sum(
            1 for p in closed_positions.values() if p.get("realized_pnl", 0) > 0
        )
        total_profit = sum(
