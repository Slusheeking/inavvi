...
class StockScreener:
        ...
        if not watchlist:
            ...
            if pre_market_movers:
                ...
                if gainers_losers:
                    ...
                    candidates = []
                    candidates.extend([item['symbol'] for item in gainers_losers.get('gainers', [])[:15]])
                    candidates.extend([item['symbol'] for item in gainers_losers.get('losers', [])[:15]])
                else:
                    # Use static list of liquid stocks as last resort
                    logger.info("Using default stock list for initial watchlist")
                    candidates = ['AAPL', 'MSFT', 'AMZN', 'GOOGL', 'META', 'TSLA', 'NVDA', 'AMD', 'NFLX', 'PYPL',
                                 'DIS', 'BA', 'JPM', 'V', 'WMT', 'PFE', 'XOM', 'BAC', 'T', 'INTC']
    ...
    async def analyze_symbol(self, symbol: str) -> Optional[Dict]:
        ...
        if not intraday_data is not None and isinstance(intraday_data, pd.DataFrame) and intraday_data.empty:
            return None
    ...
    async def get_pre_market_movers(self):
        ...
        if not pre_market_movers:
            ...
            if pre_market_movers:
                redis_client.set("market:pre_market_movers", pre_market_movers, expiry=900)  # 15 min expiry
    ...
    async def get_market_movers(self):
        ...
        if not gainers_losers:
            ...
            if gainers_losers:
                redis_client.set("market:gainers_losers", gainers_losers, expiry=300)  # 5 min expiry
    ...
    def filter_candidates_by_indicator(self, indicator: str, min_value: float, max_value: float) -> List[Dict]:
        """
    ...
    def get_bullish_candidates(self) -> List[Dict]:
        ...
        return [c for c in candidates if (
            c.get('rsi_14', 50) > 50 and
            c.get('macd_histogram', 0) > 0 and
            c.get('pattern') in ['breakout', 'continuation', 'flag'] and
            c.get('pattern_confidence', 0) >= 0.6
        )]
    ...
    def get_bearish_candidates(self) -> List[Dict]:
        ...
        return [c for c in candidates if (
            c.get('rsi_14', 50) < 50 and
            c.get('macd_histogram', 0) < 0 and
            c.get('pattern') in ['reversal', 'head_shoulders', 'double_top'] and
            c.get('pattern_confidence', 0) >= 0.6
        )]
