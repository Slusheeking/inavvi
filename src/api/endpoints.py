...
async def get_market_movers(category: str = Query("gainers", enum=["gainers", "losers", "active", "premarket"]), 
...
async def update_watchlist(request: WatchlistUpdateRequest):
    ...
    try:
        ...
        return {"message": f"Watchlist updated with action: {request.action}", "watchlist": new_watchlist}
    except Exception as e:
...
@router.get("/logs")
async def get_logs(level: str = Query("info", enum=["debug", "info", "warning", "error"]), 
    ...
    try:
        ...
        for line in reversed(lines):  # Start from the end
            ...
            if len(parts) >= 3:
                ...
                if level == "debug" or line_level in ["critical", "error", "warning"] or (level == "info" and line_level == "info"):
