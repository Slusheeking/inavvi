...
class DataPreparation:
    ...
    async def download_training_data(self, symbols: List[str], 
                                    days: int = 180, 
                                    timeframe: str = 'day',
        ...
        for symbol in symbols:
            ...
            try:
                if timeframe == 'day':
                    ...
                    df = await polygon_client.get_intraday_bars(symbol, minutes=1, days=min(days, 5))
                else:
    ...
    def create_technical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in [5, 10, 20, 50]:
            df_features[f'close_to_sma_{period}'] = df_features['close'] / df_features[f'sma_{period}'] - 1
            df_features[f'close_to_ema_{period}'] = df_features['close'] / df_features[f'ema_{period}'] - 1
        ...
        for period in [20]:
            ...
            df_features[f'bb_width_{period}'] = (df_features[f'bb_upper_{period}'] - df_features[f'bb_lower_{period}']) / middle
    ...
    def create_pattern_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in ['open', 'high', 'low', 'close']:
            ...
            df_features[f'{col}_norm'] = (df_features[col] - rolling_min) / (rolling_max - rolling_min + 1e-8)
    ...
    def create_target_variables(self, df: pd.DataFrame, lookahead_periods: int = 5, threshold: float = 0.02) -> pd.DataFrame:
        """
        ...
        df_with_targets[f'future_return_{lookahead_periods}'] = future_close / df_with_targets['close'] - 1
        
        # Create binary target variables
        df_with_targets[f'target_up_{lookahead_periods}'] = df_with_targets[f'future_return_{lookahead_periods}'] > threshold
        df_with_targets[f'target_down_{lookahead_periods}'] = df_with_targets[f'future_return_{lookahead_periods}'] < -threshold
        
        # Convert to integer for classification
        df_with_targets[f'target_up_{lookahead_periods}'] = df_with_targets[f'target_up_{lookahead_periods}'].astype(int)
        df_with_targets[f'target_down_{lookahead_periods}'] = df_with_targets[f'target_down_{lookahead_periods}'].astype(int)
        
        # Multi-class target (-1 for down, 0 for neutral, 1 for up)
        df_with_targets[f'target_direction_{lookahead_periods}'] = 0
        df_with_targets.loc[df_with_targets[f'target_up_{lookahead_periods}'] == 1, f'target_direction_{lookahead_periods}'] = 1
        df_with_targets.loc[df_with_targets[f'target_down_{lookahead_periods}'] == 1, f'target_direction_{lookahead_periods}'] = -1
    ...
    def prepare_training_dataset(self, data: Dict[str, pd.DataFrame], 
        ...
        os.makedirs(os.path.join(settings.data_dir, 'processed'), exist_ok=True)
        combined_df.to_csv(os.path.join(settings.data_dir, 'processed', f'{feature_type}_features.csv'))
        
        # Get feature names
        target_col = f'target_direction_{lookahead_periods}'
        feature_cols = [col for col in combined_df.columns if col not in 
                       [target_col, f'future_return_{lookahead_periods}', 
                        f'target_up_{lookahead_periods}', f'target_down_{lookahead_periods}']]
        
        # Handle non-numeric columns (like 'symbol')
        numeric_features = combined_df[feature_cols].select_dtypes(include=[np.number]).columns.tolist()
        ...
        logger.info(f"Created dataset with {X_train.shape[0]} training and {X_test.shape[0]} testing samples")
        
        return X_train_scaled, X_test_scaled, y_train, y_test, numeric_features
    
    def prepare_pattern_dataset(self, data: Dict[str, pd.DataFrame], window_size: int = 20) -> Tuple:
        """
        ...
        for symbol, df in data.items():
            ...
            try:
                ...
                for i in range(len(df_features) - window_size):
                    ...
                    if (window['close'].iloc[-1] > window['high'].iloc[:-1].max()) and (window['volume'].iloc[-1] > window['volume'].iloc[:-1].mean() * 1.5):
                        pattern = 1  # breakout
                    
                    # Check for reversal: price changes direction
                    elif ((window['close'].pct_change().iloc[-3:].mean() * window['close'].pct_change().iloc[-6:-3].mean()) < 0) and abs(window['close'].pct_change().iloc[-3:].mean()) > 0.02:
                        pattern = 2  # reversal
                    
                    # Check for continuation: price continues in the same direction after a pause
                    elif (abs(window['close'].pct_change().iloc[-10:-5].mean()) < 0.01) and (abs(window['close'].pct_change().iloc[-5:].mean()) > 0.01) and ((window['close'].pct_change().iloc[-5:].mean() * window['close'].pct_change().iloc[-15:-10].mean()) > 0):
                        pattern = 3  # continuation
                    
                    # Check for flag pattern: short consolidation in a trend
                    elif (abs(window['close'].pct_change().iloc[-5:].mean()) < 0.01) and (abs(window['close'].pct_change().iloc[-10:-5].mean()) > 0.02):
                        pattern = 4  # flag
                    
                    # Check for triangle: decreasing volatility
                    elif (window['bb_width_20'].iloc[-1] < window['bb_width_20'].iloc[-5]) and (window['bb_width_20'].iloc[-5] < window['bb_width_20'].iloc[-10]):
                        pattern = 5  # triangle
    ...
    def prepare_sentiment_dataset(self, news_data: List[Dict], labels: Optional[List[int]] = None) -> Tuple:
        """
        ...
        if len(labels) != len(texts):
            logger.error(f"Number of labels ({len(labels)}) doesn't match number of texts ({len(texts)})")
            return texts, None
                ...
                for i in range(20, len(df_features) - 20):
                    ...
                    for j in range(1, min(21, len(df_features) - i)):
                        ...
                        state_features = [
                            ...
                            current_price / df_features['high'].iloc[entry_idx:current_idx].max() - 1,  # Price relative to high
                            current_price / df_features['low'].iloc[entry_idx:current_idx].min() - 1,   # Price relative to low
                            ...
                            df_features['volume_ratio_5'].iloc[current_idx] / 5      # Volume ratio (normalized)
                        ]
                        ...
                        if profit_pct >= 3.0:
                            ...
                            reward = 0.1 if profit_pct > 0 else -0.1  # Small reward/penalty based on P&L
