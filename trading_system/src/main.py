...
app = FastAPI(
    ...
    description="Day Trading System API"
)
...
async def rank_watchlist():
    ...
    for symbol in watchlist:
        try:
            ...
            # Add to candidates
            redis_client.add_candidate_score(symbol, score, {
    ...
    for symbol in watchlist:
        ...
        if price_data:
            watchlist_data.append({
                "symbol": symbol,
                "price": price_data.get('price', {}).get('last', 0),
                "change": price_data.get('price', {}).get('last', 0) - price_data.get('price', {}).get('open', 0),
...
if __name__ == "__main__":
    ...
    gainers_losers = await polygon_client.get_gainers