...
class YahooFinanceAPI:
        ...
        try:
            ...
            if "rate limit" in str(e).lower():
                raise YahooFinanceRateLimitError(f"Rate limit exceeded for {symbol}: {e}") from e
                 ...
                 raise YahooFinanceSymbolNotFoundError(f"Symbol not found or cannot be fetched: {symbol}") from e
            else:
                raise YahooFinanceDataError(f"Yahoo Finance error fetching ticker info for {symbol}: {e}") from e
        except Exception as e:
            logger.error(f"Unexpected error fetching ticker info for {symbol}: {e}")
            raise YahooFinanceDataError(f"Unexpected error fetching ticker info for {symbol}: {e}") from e
        ...
        try:
            ...
            if "rate limit" in str(e).lower():
                raise YahooFinanceRateLimitError(f"Rate limit exceeded for {symbol}: {e}") from e
                 ...
                 raise YahooFinanceInvalidIntervalError(f"Invalid interval provided for {symbol}: {e}") from e
            elif "cannot find or fetch ticker" in str(e).lower():
                 raise YahooFinanceSymbolNotFoundError(f"Symbol not found or cannot be fetched: {symbol}") from e
            else:
                raise YahooFinanceDataError(f"Yahoo Finance error fetching historical prices for {symbol}: {e}") from e
        except Exception as e:
            logger.error(f"Unexpected error fetching historical prices for {symbol}: {e}")
            raise YahooFinanceDataError(f"Unexpected error fetching historical prices for {symbol}: {e}") from e
    ...
    async def get_intraday_prices(self, symbol: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if "rate limit" in str(e).lower():
                ...
                raise YahooFinanceDataError(f"Yahoo Finance error fetching intraday prices for {symbol}: {e}") from e
        except Exception as e:
            logger.error(f"Unexpected error fetching intraday prices for {symbol}: {e}")
            raise YahooFinanceDataError(f"Unexpected error fetching intraday prices for {symbol}: {e}") from e
    ...
    async def get_financials(self, symbol: str) -> Optional[Dict[str, pd.DataFrame]]:
        ...
        try:
            ...
            if "rate limit" in str(e).lower():
                ...
                raise YahooFinanceDataError(f"Yahoo Finance error fetching financials for {symbol}: {e}") from e
        except Exception as e:
            logger.error(f"Unexpected error fetching financials for {symbol}: {e}")
            raise YahooFinanceDataError(f"Unexpected error fetching financials for {symbol}: {e}") from e
    ...
    async def get_earnings(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if "rate limit" in str(e).lower():
                ...
                raise YahooFinanceDataError(f"Yahoo Finance error fetching earnings for {symbol}: {e}") from e
        except Exception as e:
            logger.error(f"Unexpected error fetching earnings for {symbol}: {e}")
            raise YahooFinanceDataError(f"Unexpected error fetching earnings for {symbol}: {e}") from e
    ...
    async def get_analyst_recommendations(self, symbol: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if "rate limit" in str(e).lower():
                ...
                raise YahooFinanceDataError(f"Yahoo Finance error fetching analyst recommendations for {symbol}: {e}") from e
        except Exception as e:
            logger.error(f"Unexpected error fetching analyst recommendations for {symbol}: {e}")
            raise YahooFinanceDataError(f"Unexpected error fetching analyst recommendations for {symbol}: {e}") from e
    ...
    async def get_analyst_price_targets(self, symbol: str) -> Optional[Dict[str, float]]:
        ...
        try:
        ...
        except (YahooFinanceDataError, YahooFinanceRateLimitError, YahooFinanceSymbolNotFoundError) as e:
            ...
            raise YahooFinanceDataError(f"Unexpected error fetching analyst price targets for {symbol}: {e}") from e
    ...
    async def get_institutional_holders(self, symbol: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if "rate limit" in str(e).lower():
                ...
                raise YahooFinanceDataError(f"Yahoo Finance error fetching institutional holders for {symbol}: {e}") from e
        except Exception as e:
            logger.error(f"Unexpected error fetching institutional holders for {symbol}: {e}")
            raise YahooFinanceDataError(f"Unexpected error fetching institutional holders for {symbol}: {e}") from e
    ...
    async def get_major_holders(self, symbol: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if "rate limit" in str(e).lower():
                ...
                raise YahooFinanceDataError(f"Yahoo Finance error fetching major holders for {symbol}: {e}") from e
        except Exception as e:
            logger.error(f"Unexpected error fetching major holders for {symbol}: {e}")
            raise YahooFinanceDataError(f"Unexpected error fetching major holders for {symbol}: {e}") from e
    ...
    async def get_options_chain(self, symbol: str, expiration_date: Optional[str] = None) -> Optional[Dict[str, pd.DataFrame]]:
        ...
        try:
            ...
            if "rate limit" in str(e).lower():
                ...
                raise YahooFinanceDataError(f"Yahoo Finance error fetching options chain for {symbol}: {e}") from e
        except Exception as e:
            logger.error(f"Unexpected error fetching options chain for {symbol}: {e}")
            raise YahooFinanceDataError(f"Unexpected error fetching options chain for {symbol}: {e}") from e
    ...
    async def get_option_expirations(self, symbol: str) -> Optional[List[str]]:
        ...
        try:
            ...
            if "rate limit" in str(e).lower():
                ...
                raise YahooFinanceDataError(f"Yahoo Finance error fetching option expirations for {symbol}: {e}") from e
        except Exception as e:
            logger.error(f"Unexpected error fetching option expirations for {symbol}: {e}")
            raise YahooFinanceDataError(f"Unexpected error fetching option expirations for {symbol}: {e}") from e
    ...
    async def screen_stocks(self, criteria: Dict[str, Any], limit: int = 100) -> List[str]:
        ...
        try:
                ...
                for symbol, info in zip(symbols, results):
                    ...
                    if isinstance(info, Exception):
                        if isinstance(info, YahooFinanceSymbolNotFoundError):
                            ...
                            logger.warning(f"Rate limit hit during screening for {symbol}, will retry batch")
                            # Wait and retry this batch on rate limit
                            await asyncio.sleep(5)
                            break
                        else:
                            logger.warning(f"Failed to fetch info for {symbol} during screening: {info}")
            ...
            # Attempt to classify specific Yahoo Finance errors
            if "rate limit" in str(e).lower():
                raise YahooFinanceRateLimitError(f"Rate limit exceeded during screening: {e}") from e
            else:
    ...
    async def _get_default_universe(self) -> List[str]:
        ...
        try:
            ...
            # Attempt to classify specific Yahoo Finance errors
            if "rate limit" in str(e).lower():
                raise YahooFinanceRateLimitError(f"Rate limit exceeded fetching default universe: {e}") from e
            else:
                raise YahooFinanceDataError(f"Yahoo Finance error fetching default universe: {e}") from e
        except Exception as e:
            ...
            fallback_symbols = [
                ...
                "NFLX", "INTC", "VZ", "CSCO", "PFE", "KO", "PEP", "WMT", "MRK"
            ]
...
async def fetch_yahoo_finance_data(symbol: str, interval: str = 'day', **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        # Fetch the data
        df = await yahoo_finance_client.get_historical_prices(symbol, period=period, interval=yf_interval)
