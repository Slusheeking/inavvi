...
class ConnectionManager:
    ...
    def __init__(self):
        ...
        default_channels = [
            "system_status", "market_status", "watchlist", 
            "positions", "trades", "indicators"
        ]
    ...
    def unsubscribe(self, websocket: WebSocket, channels: List[str]):
        ...
        logger.debug(f"Client {self.client_info[websocket]['client_id']} unsubscribed from {channels}")
...
async def heartbeat_monitor():
    ...
    while True:
        try:
            ...
            for conn in list(manager.active_connections):  # Copy to avoid modification during iteration
...
async def broadcast_watchlist_update(watchlist: List[str], added: List[str] = None, removed: List[str] = None):
    """
