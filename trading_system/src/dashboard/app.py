...
def update_dashboard(n_intervals):
    ...
    try:
        ...
        if state == "running":
            ...
            status_component = dbc.Alert(f"System error: {system_state.get('error', 'Unknown error')}", color="danger")
        else:
        ...
        if market_status:
            ...
            if exchanges:
                ...
                for exchange_name, exchange_data in exchanges.items():
                    ...
                    color = "success" if status == "open" else "danger" if status == "closed" else "warning"
                    exchange_list.append(html.Li(f"{exchange_name}: ", html.Span(status, className=f"text-{color}")))
            ...
            for symbol, position_data in positions.items():
                ...
                position_card = dbc.Card([
                    ...
                    dbc.CardBody([
                        ...
                        html.P([
                            html.Span("P&L: "),
                            html.Span(f"${pnl:.2f} ({pnl_pct:.2f}%)", className=f"fw-bold text-{pnl_color}"),
                        ]),
                    ]),
                ], className="mb-2")
        ...
        if watchlist:
            ...
            for symbol in watchlist:
                ...
                if price_data and 'price' in price_data:
                    ...
                    row = html.Tr([
                        ...
                        html.Td(f"${last:.2f}"),
                        html.Td([
                            html.Span(f"{change:.2f} ({change_pct:.2f}%)", className=f"text-{change_color}"),
                        ]),
                        html.Td(f"{volume:,}"),
                        html.Td([
                            dbc.Button("Chart", color="primary", size="sm", className="me-1", id={"type": "chart-button", "index": symbol}),
                            dbc.Button("Trade", color="success", size="sm", id={"type": "trade-button", "index": symbol}),
                        ]),
                    ])
            ...
            # Create table
            if table_rows:
                watchlist_component = dbc.Table(table_header + [html.Tbody(table_rows)], striped=True, bordered=True, hover=True, responsive=True)
            else:
                watchlist_component = html.P("No watchlist data available")
        else:
            watchlist_component = html.P("Watchlist is empty")
        
        return status_component, system_stats, market_stats, positions_component, watchlist_component
...
def update_candidates_table(n_intervals):
    ...
    try:
        ...
        # Create table
        if table_rows:
            return dbc.Table(table_header + [html.Tbody(table_rows)], striped=True, bordered=True, hover=True, responsive=True)
        else:
