...
class TestPositionMonitor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_price_data = {
            "symbol": "AAPL",
            "price": {"last": 155.0, "open": 150.0, "high": 156.0, "low": 149.0, "volume": 1000000},
            "timestamp": datetime.now().isoformat(),
        }
    ...
    def test_monitor_positions(self, mock_get_positions, mock_monitor_position):
        ...
        mock_get_positions.return_value = {
            ...
            "MSFT": self.sample_position.copy(),
        }
    ...
    def test_check_forced_exit_conditions(
        self, mock_update_pnl, mock_get_indicators, mock_get_data, mock_check_forced
    ):
        ...
        # Run test asynchronously
        result = asyncio.run(
            self.monitor.check_forced_exit_conditions("AAPL", self.sample_position, "open", 3.5)
        )
    ...
    def test_monitor_position(
        self, mock_update_pnl, mock_get_position, mock_get_indicators, mock_get_data, mock_evaluate
    ):
        ...
        # Run test asynchronously
        asyncio.run(self.monitor.monitor_position("AAPL", self.sample_position, "open", 3.5))
    ...
    def test_get_exit_decision(self, mock_chat_completion, mock_get_decision):
        ...
        # Run test asynchronously
        result = asyncio.run(
            self.monitor.get_exit_decision(
                self.sample_position,
                self.sample_price_data,
                self.sample_market_context,
                self.sample_exit_recommendation,
...
class TestTradeExecutor(unittest.TestCase):
    ...
    def test_execute_trade(
        ...
        # Set up mocks
        mock_get_data.return_value = {"symbol": "AAPL", "price": {"last": 150.0}}
    ...
    def test_execute_exit(
        self, mock_delete_position, mock_set_closed, mock_set_active, mock_submit_order
    ):
        ...
        # Run test asynchronously
        result = asyncio.run(
            self.executor.execute_exit("AAPL", self.sample_position, 1.0, "take_profit")
        )

        # Assert
        self.assertTrue(result)
        mock_submit_order.assert_called_once_with(
            symbol="AAPL", qty=10, side="sell", type="market", time_in_force="day"
        )
    ...
    def test_sync_positions(
        self, mock_delete, mock_set_closed, mock_set_active, mock_get_positions, mock_list_positions
    ):
